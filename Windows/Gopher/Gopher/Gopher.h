...
private:
	...
	const int SCROLL_DEAD_ZONE = 3000; // Right thumbstick should be less sensitive.
	const int TRIGGER_DEAD_ZONE = 0;
	const int SCROLL_SPEED = 1; // Speed at which you scroll page.
	const int FPS = 150;
	const int SLEEP_AMOUNT = 1000/FPS; // number of milliseconds to sleep per iteration

	XINPUT_STATE _currentState;

	const float SPEED_ULTRA_LOW = 0.005f;
	const float SPEED_LOW = 0.015f;
	const float SPEED_MED = 0.030f;
	const float SPEED_HIGH = 0.055f;
	...
	DWORD CONFIG_SPEED_CLUTCH = NULL;
