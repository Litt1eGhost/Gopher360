...
void Gopher::loadConfigFile()
{
	...
	CONFIG_SPEED_CLUTCH = strtol(cfg.getValueOfKey<std::string>("CONFIG_SPEED_CLUTCH").c_str(), 0, 0);
...
void Gopher::loop() {
	...
	//Will slow down the pointer when holding the clutch
	if (_xboxClickIsDown[CONFIG_SPEED_CLUTCH] && (speed == SPEED_HIGH || speed == SPEED_MED || speed == SPEED_LOW)) {
		printf("Setting speed to ULTRA LOW...\n");
		prev_speed = speed;
		speed = SPEED_ULTRA_LOW;
	}
	
	if (_xboxClickIsUp[CONFIG_SPEED_CLUTCH] && speed == SPEED_ULTRA_LOW) {
		printf("Restoring speed...\n");
		speed = prev_speed;
		pulseVibrate(100, 45000, 45000);
	}
